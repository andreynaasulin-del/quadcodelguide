...
function animate(now){
  ...
  const target=coverMode
    ? {bass:.72+Math.sin(now*.0012)*.08,mid:.46+Math.sin(now*.0017+1)*.07,high:.28+Math.sin(now*.0023+2)*.05}
    : {bass:bandEnergy(28,180),mid:bandEnergy(180,2200),high:bandEnergy(2200,12000)};
...
if(new URLSearchParams(location.search).has('cover')){
  mode='cover'; playing=true; started=true;
  ui.start.hidden=true; ui.play.disabled=true;
  ui.state.textContent='SIGNAL LIVE'; ui.dot.classList.add('live');
  ui.track.textContent='IDENTITY SIGNAL / COVER';
}
window.__ARID.ready=true;requestAnimationFrame(animate);
