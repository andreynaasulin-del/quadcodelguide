...
async function uploadFileToGitHub(localPath, repoPath) {
  ...
  const putRes = await fetch(`https://api.github.com/repos/${GH_REPO}/contents/${repoPath}`, {
    ...
    body: JSON.stringify({ message: `feat: SNAP/01 widget_iframe support in landing.html`, content, sha, branch: BRANCH }),
  });
  const json = await putRes.json();
  if (!putRes.ok) { console.error('Upload failed:', json); process.exit(1); }
  console.log(`OK: ${repoPath} (commit ${json.commit?.sha})`);
}

uploadFileToGitHub('snap01/08-technical-spec.png', 'ui_views/assets/snap01-08-bom-panel.png');
